 ...
 *
 *
 *   85: class txsg_base extends tslib_pibase
 *  118:     function main($content,$conf)
 *  235:     function preprocessPluginMode($pluginMode)
 *  265:     function switchPluginMode ($pluginMode)
 *  445:     function setDefaultSearch ()
 *  488:     function DoSearch($pluginMode=1)
 *  526:     function getSearchBox($pluginMode)
 *  584:     function getDbUserSearchFields ($dbName,$PCA,$piVarSearch,$markers=Array(),$dbg=0)
 *  594:     function DoList()
 *  680:     function getResultList($orderBy='',$limitTo='',$restrict='')
 * 1015:     function DoSingle()
 * 1054:     function getShowEdit()
 * 1674:     function doLatestList ()
 * 1706:     function doLatestView ()
 * 1761:     function doCatMenu ()
 * 1831:     function listCatMenu(&$idList,$level)
 * 1896:     function DoSimpleBasket()
 * 1908:     function getRow ($uid)
 * 1983:     function getEditButtons ($mode,$row,&$PCA,$listitle='')
 * 2103:     function processHideDelete ($mode)
 * 2214:     function DoImport()
 * 2231:     function DoInfo()
 * 2274:     function DoServiceLinks()
 * 2287:     function getMoreMenuMarkers($row,$PCA)
 * 2300:     function getMoreMarkers($row,$PCA)
 * 2313:     function getAckMarkers($row,$PCA)
 * 2324:     function checkAck()
 * 2337:     function getListSummaryMarkers($r,$PCA)
 * 2355:     function getLocalDbRow($mainTable,$n,$row,$PCA, $markers=Array(),$em,$dbg=0,$mode=0)
 * 2384:     function getLocalHeaders($table,$PCA,$rowHeaders=Array(),$dbg=0)
 * 2413:     function localCheckDbRowForSave ($dbName,&$row,&$PCA,&$errors)
 * 2426:     function localPrepareDbRow ($dbName,&$row,&$PCA)
 * 2441:     function prepareNewRecord ($dbName,&$row,&$PCA)
 * 2455:     function checkListOutput ($mode,$row,$PCA,$markers)
 * 2465:     function getQuotas ()
 * 2505:     function checkQuotas ($myQuota)
 * 2555:     function checkForIframe($row)
 * 2564:     function registerAjaxFunctions ()
 * 2578:     function preprocessAjax ()
 * 2594:     function xajax_process_DemoData ($data)
 * 2620:     function pi_linkTP_URL($urlParameters=array(),$cache=0,$altPageId=0)
 * 2638:     function getSingleText ($name,$conf,$TSkey='__')
 *
 * TOTAL FUNCTIONS: 41
