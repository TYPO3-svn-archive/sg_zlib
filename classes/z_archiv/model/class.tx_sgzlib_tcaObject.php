...
/**
 * Abstract class tx_sgzlib_tcaobject
 ...
 * @since	2008-03-20
 */
class tx_sgzlib_tcaObject implements ArrayAccess, IteratorAggregate {
	 ...
	 * @author 	Fabrizio Branca <mail@fabrizio-branca.de>
	 */
	public function __construct($table = '', $uid = '', array $dataArr = array(), $ignoreEnableFields = false) {
		$this->_table = $table; 
	...
	public function loadSelf($uid, $ignoreEnableFields = false) {
		...
		$dataArr = tx_sgzlib_tcaobjectAccessor::selectByUid($uid, $this->_table, $ignoreEnableFields);
		if (is_array($dataArr)) {
			$this->setDataArray($dataArr);
		} else {
	...
	public function storeSelf($updateSubitems = false, $pidForMmRecords = 0) {
		...
		$uid = tx_sgzlib_tcaobjectAccessor::store($this->_table, $dataArray);
		$this->__set('uid', $uid);
		...
		foreach (array_keys($this->_properties) as $property) {
			if ($this->getConfig($property, 'type') == 'inline') {
				...
				$childRecords = tx_sgzlib_tcaobjectAccessor::selectByParentUid(
					$this->uid, 
					$foreign_table, 
					$foreign_field, 
					$foreign_sortby
				...
				foreach ($this[$property . '_objColl'] as $item) {
					...
					if ($isMM && !in_array($subuid, $existingChilds)) {
						...
						tx_sgzlib_tcaobjectAccessor::store($foreign_table, $dataArr);
				...
				if ($isMM) {
					foreach ($existingChilds as $child) {
						if (!in_array($child, $addedChilds)) {
							...
							tx_sgzlib_tcaobjectAccessor::deleteWhere($foreign_table, $where);
						}
					}
				}
	...
	public function deleteSelf($markOnlyAsDeleted = true) {
		...
		if ($markOnlyAsDeleted) {
			if (($deletedFieldName = $this->getSpecialField('delete')) !== false) {
				tx_sgzlib_tcaobjectAccessor::update($this->_table, array($deletedFieldName => 1, 'uid' => $this->uid));
			} else {
			...
			tx_sgzlib_tcaobjectAccessor::delete($this->_table, $this->uid);
		}
	} // end method
	 ...
	 * @return 	tx_sgzlib_tcaobjectCollection 	object collection
	...
	protected function processModifier_objColl($property, $calledProperty) {
		...
		if ($type == 'inline') {
			...
			$propertyCollectionName = class_exists($propertyCollectionName) ? $propertyCollectionName : 'tx_sgzlib_tcaobjectCollection';
			
			$value = new $propertyCollectionName(); /* @var $value tx_sgzlib_tcaobjectCollection */
			...
			$dataArr = tx_sgzlib_tcaobjectAccessor::selectByParentUid(
				$this->uid, 
				$foreign_table,  
				$foreign_field, 
				$this->getConfig($property, 'foreign_sortby')
			...
			if (!empty($dataArr)) {
				...
				foreach ($dataArr as $data) {
					try {	
						...
						$tmpObj = $isMM ? new $classname($data[$foreign_mm_field]) : new $classname('', $data); /* @var $tmpObj tx_sgzlib_tcaobject */
	 ...
	 * @param 	string		property name
	 * @param 	string		property name that was originally called
	 * @return 	tx_sgzlib_tcaobject 	object
	 * @throws	tx_pttools_exception
	 * @author	Fabrizio Branca <branca@punkt.de>
	 * @since	2008-04-27
	 */
	...
	protected function __call($methodName, $parameters) {
		...
		if (in_array($prefix, array('get', 'set'))) {
			...
			$dataArr = tx_sgzlib_tcaobjectAccessor::selectCollection($this->_table, $where, 1);
			if (is_array($dataArr)) {
				$this->setDataArray($dataArr);
			} else {
	...
	public function offsetExists($offset) {
		...
		return true; // key_exists($property, $this->_properties);
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sgzlib/classes/model/class.tx_sgzlib_tcaObject.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sgzlib/classes/model/class.tx_sgzlib_tcaObject.php']);
}
