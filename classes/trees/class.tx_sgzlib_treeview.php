...
*
*  -- How to add (Note: Cats _must_ be MM):
*  1. add to ext_tables.php
*     // class for displaying the category tree in BE forms.
*     include_once(t3lib_extMgm::extPath($_EXTKEY).'class.tx_mimasp_treeview.php');
*  2. add to tca.php
*     'cat' => Array ( ... 'config' => Array ( ...
*				'type' => 'select',	
*				'form_type' => 'user',	
*				'userFunc' => 'tx_mimasp_treeview->displayCategoryTree',
*				'treeView' => 1,	
*				'foreign_table' => 'tx_mimasp_cat',	
*				'autoSizeMax' => $confArr['categoryTreeHeigth']>3 ? $confArr['categoryTreeHeigth'] : 8,
*  3. $TCA["tx_mimrezept_cat"] = Array ( ... "ctrl" => Array (...
 ...
 *
 *
 *   60: class tx_mimrezept_tceFunc_selectTreeView extends t3lib_treeview
 *   72:     function wrapTitle($title,$v)
 *
 *
 *   91: class tx_mimrezept_treeview
...
require_once(PATH_t3lib.'class.t3lib_treeview.php');
	 ...
	 *
	 */
class tx_sgzlib_tceFunc_selectTreeView extends t3lib_treeview {
	...
	function wrapTitle($title,$v)	{
		if($v['uid']>0) {
			if (in_array($v['uid'],$this->TCEforms_nonSelectableItemsArray)) {
				return '<a href="#"><span style="color:#999;cursor:default;">'.$title.'</span></a>';
			} else {
	...
	/**
	 * this class displays a tree selector with nested mim_rezept categories.
	 *
	 */
class tx_sgzlib_treeview {
	var $mainTable = 'tx_demo';
	var $catTable = 'tx_democat_cat';
	var $extKey = 'sg_zlib';	// The extension key.
	...
	function displayCategoryTree($PA, $fobj)    {
		...
		$this->excludeList = $GLOBALS['BE_USER']->getTSConfigVal($this->extKey.'Perms.'.$this->catTable.'.excludeList');
// 		if ($this->excludeList) {
// 			$removeItems .= ','.$this->excludeList;
// 		}
		$this->includeList = $GLOBALS['BE_USER']->getTSConfigVal($this->extKey.'Perms.'.$this->catTable.'.includeList');
		...
		if ($maxitems<=1 AND !$config['treeView'])	{
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] && ($table == $this->mainTable || $table == $this->catTable)) { 
				// the current record is a translation of another record
				if ($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf'][$this->mainTable]) { // get  extConf array
				...
				if ($confArr['useStoragePid']) {
					...
					$SPaddWhere = ' AND '.$this->catTable.'.pid IN (' . $storagePid . ')';
				}
				$errorMsg = array();
				...
				$catres = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query ($this->catTable.'.uid,'.$this->catTable.'.title,'.$this->catTable.'_mm.sorting AS mmsorting', $this->mainTable, $this->catTable.'_mm', $this->catTable, ' AND '.$this->catTable.'_mm.uid_local='.$row['l18n_parent'].$SPaddWhere,'', 'mmsorting');
				$categories = array();
				$NACats = array();
				$na = false;
				...
				if($config['treeView'] AND $config['foreign_table']) {
					...
					if ($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf'][$this->extKey]) { // get  extConf array
						$confArr = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf'][$this->extKey]);
					}
					if ($confArr['useStoragePid']) {
						$TSconfig = t3lib_BEfunc::getTCEFORM_TSconfig($table,$row);
					...
					if($config['treeViewClass'] AND is_object($treeViewObj = &t3lib_div::getUserObj($config['treeViewClass'],'user_',false)))      {
						...
						$treeViewObj = t3lib_div::makeInstance('tx_sgzlib_tceFunc_selectTreeView');
					}

					if ($this->excludeList) {
						$catlistWhere = ' AND '.$this->catTable.'.uid NOT IN ('.implode(t3lib_div::intExplode(',',$this->excludeList),',').')';
					}
...
// 					}
					...
					$width = 364; // default width for the field with the category tree
...
// 						$tvP[1] = rawurlencode($this->pObj->sL($PA['fieldTSConfig']['altLabels.'][$evalValue]));
	 ...
	 * @see tx_mimrezept_tceFunc_selectTreeView::wrapTitle()
	 */
	function getNotAllowedItems($PA,$SPaddWhere) {
		...
		$allowedItemsList=$GLOBALS['BE_USER']->getTSConfigVal($this->extKey.'Perms.'.$fTable.'.allowedItems');
...
// 				debug($catvals,__FUNCTION__);
	...
	function findRecursiveCategories ($PA,$row,$table,$storagePid,$treeIds) {
		...
		if (!$this->excludeList && !$this->includeList) {
			if ($table == 'tt_content' && $row['CType']=='list' && $row['list_type']==9) { // = tt_content element which inserts plugin 
			...
			} elseif ($table == $this->catTable || $table == $this->mainTable) {
				if ($table == $this->catTable && $row['pid'] == $storagePid && intval($row['uid']) && !in_array($row['uid'],$treeIds))	{ // if the selected category is not empty and not in the array of tree-uids it seems to be part of a chain of recursive categories
		...
		if ($GLOBALS['BE_USER']->getTSConfigVal('options.useListOfAllowedItems') && !$GLOBALS['BE_USER']->isAdmin()) {
			...
			if ($notAllowedItems[0]) {
				...
				if ($uidField) {
					...
					$catres = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query ($this->catTable.'.uid,'.$this->catTable.'.title,'.$this->catTable.'_mm.sorting AS mmsorting', $this->mainTable, $this->catTable.'_mm', $this->catTable, ' AND '.$this->catTable.'_mm.uid_local='.$uidField.$SPaddWhere,'', 'mmsorting');
					$NACats = array();
					if ($catres) {
						while ($catrow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($catres)) {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sg_zlib/classes/trees/class.tx_sgzlib_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sg_zlib/classes/trees/class.tx_sgzlib_treeview.php']);
}
?>
